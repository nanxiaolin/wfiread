...
%   startx, starty, endx, endy, frames, num_fiducials, reserved, reserved
	...
	%pref_file = fullfile('/mnt/data0/xiaolin/Temp', name);
...
%% estimate the number of particles per frame by performing
    ...
    numObject = 0; frames_in_range = 0;
    for f = 1 : userdata.frames
        if userdata.actualframes(f) >= pstart && userdata.actualframes(f) <= pend
            numObject = numObject + findparticles(0, f);
            frames_in_range = frames_in_range + 1;
        end
    end
    %frames_in_range
    est_particles = ceil(numObject * numframes / frames_in_range);
...
%% prepare the TIFF object and position the IFD pointer
    showmsg(h_mainfig, 'message', 'Opening and analyzing TIFF file ...');
    pause(0.01);
    
    % prepare the tf structure. first, determine the 1st file to open
    file_id = 1;
    frame_count = 0;
    
    % next frame_id at which the file should be switched
    next_switch = params.fileinfo.frames(1)+1; 
    
    if params.fileinfo.numfiles > 1
        for f = 1 : params.fileinfo.numfiles - 1
           frame_count = frame_count + params.fileinfo.frames(f);

           if frame_count < pstart
               file_id = file_id + 1;
               next_switch = next_switch + params.fileinfo.frames(f+1);
           end
        end
    end
        
    % now open the first file
    try
        tf = Tiff( params.fileinfo.filenames{file_id} );
    catch ME
        showmsg(h_mainfig, 'message', 'Error opening the TIFF file. Re-load the file and try again?');
        clear;
        return;
    end
    
    % position the IFD at the right position
    ifd_offset = params.fileinfo.frames(file_id) - (next_switch - pstart) + 1;
    tf.setDirectory(ifd_offset);
...
%% main loop for reading and analyzing the raw image frames
    ...
    for ifd = pstart : pend
		...
		tempcoords(numparticles+1 : numparticles+numObject, 2) = output(3, 1:numObject);				% x. 
		tempcoords(numparticles+1 : numparticles+numObject, 3) = output(4, 1:numObject);				% y. 
        ...
        if (counted_frames == 1 || (mod(counted_frames, 20) == 0) || (counted_frames == numframes))
            ...
            msg = sprintf('Coordinate extraction in progress. Processing frame %d (of %d frames) with %d particles (est. total particle: %.2f M; time left: %.1f min). File ID#: %d', counted_frames, numframes, numparticles, est_particles, est_time, file_id);
            showmsg(h_mainfig, 'message', msg);
            pause(0.01);
        end

        if ifd == next_switch
            % switch file id and tf - add a console output to doublecheck
            % if the last IFD of current file has been reached.
            if tf.lastDirectory
                msg = sprintf('Reached the end of file #%d. Switching to file #%d', file_id, file_id+1);
                disp(msg);
            end
            
            tf.close;
            file_id = file_id + 1;
            next_switch = next_switch + params.fileinfo.frames(file_id);
